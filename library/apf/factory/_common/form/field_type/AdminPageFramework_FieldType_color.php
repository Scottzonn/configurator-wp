...
abstract class CConfiguratorAdminPageFramework_FieldType_Base extends CConfiguratorAdminPageFramework_Form_Utility {
    ...
    public function __construct($asClassName = 'admin_page_framework', $asFieldTypeSlug = null, $oMsg = null, $bAutoRegister = true) {
        ...
        $this->oMsg = $oMsg ? $oMsg : CConfiguratorAdminPageFramework_Message::getInstance();
        if ($bAutoRegister) {
    ...
    protected function getFieldOutput(array $aFieldset) {
        ...
        $_oFieldset = new CConfiguratorAdminPageFramework_Form_View___Fieldset($aFieldset, $_oCallerForm->aSavedData, $_oCallerForm->getFieldErrors(), $_oCallerForm->aFieldTypeDefinitions, $_oCallerForm->oMsg, $_oCallerForm->aCallbacks);
    ...
    protected function enqueueMediaUploader() {
        ...
        if (function_exists('wp_enqueue_media')) {
            new CConfiguratorAdminPageFramework_Form_View___Script_MediaUploader($this->oMsg);
        } else {
    ...
    abstract class CConfiguratorAdminPageFramework_FieldType extends CConfiguratorAdminPageFramework_FieldType_Base {
        public function _replyToFieldLoader() {
            $this->setUp();
        }
        public function _replyToGetScripts() {
    ...
    class CConfiguratorAdminPageFramework_FieldType_color extends CConfiguratorAdminPageFramework_FieldType {
        ...
        protected function getStyles() {
            return ".repeatable .colorpicker {display: inline;}.sz-field-color .wp-picker-container {vertical-align: middle;}.sz-field-color .ui-widget-content {border: none;background: none;color: transparent;}.sz-field-color .ui-slider-vertical {width: inherit;height: auto;margin-top: -11px;}.sz-field-color .sz-repeatable-field-buttons {margin-top: 0;}.sz-field-color .wp-color-result {margin: 3px;}";
        }
        protected function getScripts() {
            $_aJSArray = json_encode($this->aFieldTypeSlugs);
            $_sDoubleQuote = '\"';
            return <<<JAVASCRIPTS
registerCConfiguratorAdminPageFrameworkColorPickerField = function( osTragetInput, aOptions ) {
    ...
    jQuery().registerCConfiguratorAdminPageFrameworkCallbacks( {     
        ...
        repeated_field: function( oCloned, aModel ) {
            ...
            oCloned.find( 'input.input_color' ).each( function( iIterationIndex ) {
                ...
                registerCConfiguratorAdminPageFrameworkColorPickerField( _oNewColorInput );                
...
JAVASCRIPTS;
        ...
        protected function getField($aField) {
            ...
            return $aField['before_label'] . "<div class='sz-input-label-container'>" . "<label for='{$aField['input_id']}'>" . $aField['before_input'] . ($aField['label'] && !$aField['repeatable'] ? "<span " . $this->getLabelContainerAttributes($aField, 'sz-input-label-string') . ">" . $aField['label'] . "</span>" : "") . "<input " . $this->getAttributes($aField['attributes']) . " />" . $aField['after_input'] . "<div class='repeatable-field-buttons'></div>" . "</label>" . "<div class='colorpicker' id='color_{$aField['input_id']}'></div>" . $this->_getColorPickerEnablerScript("{$aField['input_id']}") . "</div>" . $aField['after_label'];
        }
...
jQuery( document ).ready( function(){
    registerCConfiguratorAdminPageFrameworkColorPickerField( '{$sInputID}' );
});            
JAVASCRIPTS;
