...
class CConfiguratorAdminPageFramework_Form_View___Script_CollapsibleSection extends CConfiguratorAdminPageFramework_Form_View___Script_Base {
...
( function( $ ) {
    ...
    jQuery( document ).ready( function() {
        jQuery( this ).initializeCConfiguratorAdminPageFrameworkCollapsibleSections();
    });              
     ...
     * @since    3.7.0
     */
    jQuery( document ).on( 'sz_saved_widget', function( event, oWidget ){
        jQuery( oWidget ).initializeCConfiguratorAdminPageFrameworkCollapsibleSections();
    });    
    
    $.fn.initializeCConfiguratorAdminPageFrameworkCollapsibleSections = function() {
        
        // Expand collapsible sections that are set not to collapse by default 
        jQuery( this ).find( '.sz-collapsible-sections-title[data-is_collapsed=\"0\"]' )
            .next( '.sz-collapsible-sections-content' )
            .slideDown( 'fast' );
        jQuery( this ).find( '.sz-collapsible-section-title[data-is_collapsed=\"0\"]' )
            .closest( '.sz-section-table' )
            .find( 'tbody' )
            .slideDown( 'fast' );
            
        // Hide collapsible sections of 'section' containers as they are somehow do not get collapsed by default.
        jQuery( this ).find( '.sz-collapsible-section-title[data-is_collapsed=\"1\"]' )
            .closest( '.sz-section-table' )
            .find( 'tbody' )
            .hide();
        
        // Bind the click event to the title element.
        jQuery( this ).find( '.sz-collapsible-sections-title, .sz-collapsible-section-title' )
            .enableCConfiguratorAdminPageFrameworkCollapsibleButton();
        
        // Insert the toggle all button.
        jQuery( this ).find( '.sz-collapsible-title[data-toggle_all_button!=\"0\"]' ).each( function(){
            
            var _oThis        = jQuery( this ); // to access from inside the below each() method.
            var _bForSections = jQuery( this ).hasClass( 'sz-collapsible-sections-title' );   // or for the 'section' container.
            ...
            jQuery.each( _aPositions, function( iIndex, _sPosition ) {
                ...
                var _sLeftOrRight = -1 !== jQuery.inArray( _sPosition, [ 'top-right', 'bottom-right', '0' ] )   // if found
                    ? 'right'   // default
                    : 'left';            
                _oButton.find( '.sz-collapsible-toggle-all-button' ).css( 'float', _sLeftOrRight );
                ...
                if ( _bForSections ) {
                    var _oTargetElement = 'before' === _sTopOrBottom
                        ? _oThis
                        : _oThis.next( '.sz-collapsible-content' );
                        _oTargetElement[ _sTopOrBottom ]( _oButton );
                } else {    // for 'section' containers
                    _oThis.closest( '.sz-section' )[ _sTopOrBottom ]( _oButton );
                }                
                ...
                _oButton.on( 'click', function(){                  
                    ...
                    var _oButtons = _bForSections
                        ? jQuery( this ).closest( '.sz-sectionset' ).siblings().andSelf().find( '> .sz-collapsible-toggle-all-button-container' )
                        : jQuery( this ).siblings( '.sz-collapsible-toggle-all-button-container' ).andSelf();
                    _oButtons.toggleClass( 'flipped' );
                    if ( _bForSections ) {
                        _oButton.parent().parent().children().children( '* > .sz-collapsible-title' ).each( function() {
                            jQuery( this ).trigger( 'click', [ 'by_toggle_all_button' ] );
                        } );
                    } else {
                        _oButton.closest( '.sz-sections' ).children( '.sz-section' ).children( '.sz-section-table' ).children( 'caption' ).children( '.sz-collapsible-title' ).each( function() {
                            jQuery( this ).trigger( 'click', [ 'by_toggle_all_button' ] );
                        } );
                    }
     ...
     */
    $.fn.enableCConfiguratorAdminPageFrameworkCollapsibleButton = function() {
        ...
        function _isFieldElement( nodeTarget ) {
            ...
            if ( jQuery( nodeTarget ).hasClass( 'sz-collapsible-button' ) ) {
                return false;
            }
        ...
        jQuery( this ).on( 'click', function( event, sContext ){
            ...
            var _sContainerType = jQuery( this ).hasClass( 'sz-collapsible-sections-title' )
                ? 'sections'
                : 'section';
            var _oTargetContent = 'sections' === _sContainerType
                ? jQuery( this ).next( '.sz-collapsible-content' ).first()
            ...
            if ( 'expand' === _sAction && _oThis.data( 'collapse_others_on_expand' ) ) {
                _oThis.parent().parent().children().children( '* > .sz-collapsible-content' ).not( _oTargetContent ).slideUp( 'fast', function() {
                    jQuery( this ).prev( '.sz-collapsible-title' ).addClass( 'collapsed' );
                });
            }
...
JAVASCRIPTS;
    ...
    static private function _getToggleAllButtonHTML($oMsg) {
        ...
        return "<div class='sz-collapsible-toggle-all-button-container'>" . "<span class='sz-collapsible-toggle-all-button button " . $_sDashIconSort . "'" . " title='" . esc_attr($_sLabelToggleAllSections) . "'>" . $_sText . "</span>" . "</div>";
    }
    }
