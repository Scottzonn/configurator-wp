...
abstract class CConfiguratorAdminPageFramework_Form_View___Fieldset_Base extends CConfiguratorAdminPageFramework_Form_Utility {
    ...
    private function _getFormatted($aFieldset, $aFieldTypeDefinitions) {
        return $this->uniteArrays($aFieldset, $this->_getFieldTypeDefaultArguments($aFieldset['type'], $aFieldTypeDefinitions) + CConfiguratorAdminPageFramework_Form_Model___Format_Fieldset::$aStructure);
    }
    ...
    private function _loadScripts($sStructureType = '') {
        if ('widget' === $sStructureType && !self::$_bIsLoadedJSScripts_Widget) {
            new CConfiguratorAdminPageFramework_Form_View___Script_Widget;
        ...
        new CConfiguratorAdminPageFramework_Form_View___Script_Utility;
        new CConfiguratorAdminPageFramework_Form_View___Script_OptionStorage;
        new CConfiguratorAdminPageFramework_Form_View___Script_AttributeUpdator;
        new CConfiguratorAdminPageFramework_Form_View___Script_RepeatableField($this->oMsg);
        new CConfiguratorAdminPageFramework_Form_View___Script_SortableField;
    }
...
jQuery( document ).ready( function() {
    var _oButtonPlaceHolders = jQuery( '#{$sFieldsContainerID} > .sz-field.without-child-fields .repeatable-field-buttons' );
    ...
    /* Otherwise, insert the button element at the beginning of the field tag */
         ...
         */
        if ( ! jQuery( '#{$sFieldsContainerID} .sz-repeatable-field-buttons' ).length ) { 
            jQuery( '#{$sFieldsContainerID} > .sz-field.without-nested-fields' ).prepend( $_sSmallButtons );
        }
        /**
         * Support for nested fields.
         * For nested fields, add the buttons to the fields tag.
         */
        jQuery( '#{$sFieldsContainerID} > .sz-field.with-nested-fields' ).prepend( $_sNestedFieldsButtons );
        
        /**
         * Support for inline mixed fields.
         */
        // jQuery( '#{$sFieldsContainerID} > .sz-field.with-mixed-fields' ).prepend( $_sNestedFieldsButtons );
        
    }     
    jQuery( '#{$sFieldsContainerID}' ).updateCConfiguratorAdminPageFrameworkRepeatableFields( $_aJSArray ); // Update the fields     
});
JAVASCRIPTS;
    ...
    private function ___getRepeatableButtonHTML($sFieldsContainerID, array $aArguments, $iFieldCount, $bSmall = true) {
        $_oFormatter = new CConfiguratorAdminPageFramework_Form_Model___Format_RepeatableField($aArguments, $this->oMsg);
    ...
    private function ___getContainerAttributes($aArguments) {
        $_aContainerAttributes = array('class' => $this->getClassAttribute('sz-repeatable-field-buttons', !empty($aArguments['disabled']) ? 'disabled' : ''),);
    ...
    protected function _getSortableFieldEnablerScript($sFieldsContainerID) {
        $_sScript = <<<JAVASCRIPTS
    jQuery( document ).ready( function() {
        jQuery( this ).enableCConfiguratorAdminPageFrameworkSortableFields( '$sFieldsContainerID' );
    });
JAVASCRIPTS;
        return "<script type='text/javascript' class='sz-sortable-field-enabler-script'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>";
    }
    }
    class CConfiguratorAdminPageFramework_Form_View___Fieldset extends CConfiguratorAdminPageFramework_Form_View___Fieldset_Base {
        public function get() {
            $_aOutputs = array();
            $_oFieldError = new CConfiguratorAdminPageFramework_Form_View___Fieldset___FieldError($this->aErrors, $this->aFieldset['_section_path_array'], $this->aFieldset['_field_path_array'], $this->aFieldset['error_message']);
            $_aOutputs[] = $_oFieldError->get();
            $_oFieldsFormatter = new CConfiguratorAdminPageFramework_Form_Model___Format_Fields($this->aFieldset, $this->aOptions);
        ...
        private function _getEachFieldOutput($aField, $isIndex, array $aCallbacks, $bIsLastElement = false) {
            ...
            $_oSubFieldFormatter = new CConfiguratorAdminPageFramework_Form_Model___Format_EachField($aField, $isIndex, $aCallbacks, $_aFieldTypeDefinition);
        ...
        private function _getFieldOutput($sContent, $aField, $bIsLastElement) {
            $_oFieldAttribute = new CConfiguratorAdminPageFramework_Form_View___Attribute_Field($aField);
        ...
        private function _getFinalOutput($aFieldset, array $aFieldsOutput, $iFieldsCount) {
            $_oFieldsetAttributes = new CConfiguratorAdminPageFramework_Form_View___Attribute_Fieldset($aFieldset);
        ...
        private function _getEmbeddedFieldTitle($aFieldset) {
            ...
            $_oFieldTitle = new CConfiguratorAdminPageFramework_Form_View___FieldTitle($aFieldset, '', $this->aOptions, $this->aErrors, $this->aFieldTypeDefinitions, $this->aCallbacks, $this->oMsg);
        ...
        private function _getChildFieldTitle($aFieldset) {
            ...
            $_oFieldTitle = new CConfiguratorAdminPageFramework_Form_View___FieldTitle($aFieldset, array('sz-child-field-title'), $this->aOptions, $this->aErrors, $this->aFieldTypeDefinitions, $this->aCallbacks, $this->oMsg);
            return $_oFieldTitle->get();
        }
        private function _getFieldsetContent($aFieldset, $aFieldsOutput, $iFieldsCount) {
            ...
            $_oFieldsAttributes = new CConfiguratorAdminPageFramework_Form_View___Attribute_Fields($aFieldset, array(), $iFieldsCount);
        ...
        private function _getExtras($aField, $iFieldsCount) {
            $_aOutput = array();
            $_oFieldDescription = new CConfiguratorAdminPageFramework_Form_View___Description($aField['description'], 'sz-fields-description');
