(function($){
    ...
    $( document ).ready( function(){
        ...
        if ( 'undefined' === typeof CConfiguratorAdminPageFrameworkSelect2FieldType ) {
            return;
        }
        debugLog( CConfiguratorAdminPageFrameworkSelect2FieldType );
        ...
        $().registerCConfiguratorAdminPageFrameworkCallbacks( {
            /**
    ...
    function initializeSelect2( oNode ) {
        ...
        if ( _aOptions[ 'search_callback' ] ) {
            ...
            _aOptions = $.extend(
                ...
                {
                    ajax: {
                        url: CConfiguratorAdminPageFrameworkSelect2FieldType.ajaxURL,
                        dataType: 'json',
                        ...
                        data: function (params) {
                            ...
                            return {
                                ...
                                doing_select2_ajax: CConfiguratorAdminPageFrameworkSelect2FieldType.nonce, // ensure it is called from here
        ...
        if ( _aOptions[ 'width' ] && 'auto' !== _aOptions[ 'width' ] ) {
            var _oFieldContainer = _oSelect2Target.closest( '.sz-field-select2' );
            _oFieldContainer.css( 'width', _aOptions[ 'width' ] );
            _oFieldContainer.children( '.sz-select-label' )
                .css( 'width', '100%' );
            _oFieldContainer.children( '.sz-select-label' )
                .children( 'label' )
                .children( '.sz-input-container' )
                .css( 'width', '100%' );
            _aOptions[ 'width' ] = '100%';
        }
        ...
        if ( _aOptions[ 'search_callback' ] ) {
             ...
             * Set initial values.
             */
            var _oInputForEncoded = _oSelect2Target.closest( '.sz-field' )
    ...
    function templateResultForIcon( state ) {
        var _oIconDefinition = $( state.element ).closest( '.sz-field' ).find( '.custom-icon-definition[data-value=' + state.id + ']' );
    ...
    function debugLog( ...message ) {
        if ( ! parseInt( CConfiguratorAdminPageFrameworkSelect2FieldType.debugMode ) ) {
            return;
        }
